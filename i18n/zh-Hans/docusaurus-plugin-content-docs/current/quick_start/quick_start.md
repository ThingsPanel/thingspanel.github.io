...
首次打开时，页面会自动跳转到 ThingsPanel 应用市场。

如果您还没有账号，请先注册；如果已有账号，可点击“立即登录”。完成登录验证后，系统会自动跳转回您已部署的 ThingsPanel 平台。

首次回到 ThingsPanel 后，您需要设置 ThingsPanel 的系统超级管理员密码。该密码与应用市场账号密码相互独立，无需保持一致。
