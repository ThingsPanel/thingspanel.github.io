...
:::info 权限说明
社区版为单租户版本，不提供租户内用户管理和按用户分配设备的能力。企业版中，角色权限负责页面和功能权限，设备详情的用户绑定负责设备数据范围；两者同时满足时，用户才能按授权操作设备。
:::
...
- 设备连接诊断功能提供可视化的设备通信健康监控，让开发者可以实时掌握设备状态。

- **统计指标监控**：持续统计设备通信全过程的成功率（上行、下行、存储）。
- **失败记录追踪**：自动保存最近的失败记录，包含时间、方向、阶段和错误描述。

![诊断面板](/img/devices/diagnosis/diagnosis_panel.png)

### 2.12、设备详情中的用户授权

企业版可以在 **设备管理 → 设备详情 → 用户** 中添加关联用户并设置设备权限。

1. 将用户关联到目标设备。
2. 将设备权限设置为 **管理（含读取）**，用户即可读取并管理该设备。
3. 如果用户未关联其他设备，则在设备管理中看不到其他用户的设备。

角色权限只决定用户能否访问页面和使用功能，设备详情中的用户绑定决定用户能看到和操作哪些设备。

![设备详情中的用户授权](./images/device-detail-management-isolation-20260919.png)
